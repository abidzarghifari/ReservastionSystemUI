...
// Gunakan .refine() untuk validasi kustom
.refine((data) => data.password === data.password_confirmation, {
  // Pesan error jika validasi gagal
  message: "Konfirmasi password tidak cocok!",
  // Terapkan error ini ke field 'confirmPassword'
...
export default function Page() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // <-- BARU: Untuk loading
  const router = useRouter(); // <-- BARU: Hook untuk navigasi
  /**
   * 2. Definisikan Form Anda
   *
   * Kita menggunakan react-hook-form dengan zodResolver
   * untuk menghubungkan skema validasi kita.
   */
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // 'values' sudah divalidasi dan aman untuk digunakan.
    // 'values' tidak mengandung 'confirmPassword' jika Anda
    // tidak mendefinisikannya di skema dasar (tapi di sini kita definisikan,
    // jadi Anda mungkin ingin menghapusnya sebelum mengirim ke API)
    
    // Kita hapus 'confirmPassword' agar tidak terkirim ke API
    ...
    try {
      ...
      const response = await api.post('web/register', dataToSend);
      ...
      // (toast.success() dan setTimeout dihapus dari sini)
  ...
  return (
	<>
					<div className="pb-5 w-full max-w-lg flex flex-col self-center" >
						...
						<p className="text-white/50 text-xs">Sign Up untuk memulai reservasi dan menikmati layanan terbaik dari kami.</p>
					</div>
